...
int	ft_caller(va_list args, t_flags flags)
{
	...
	if (!str)
		return (-1);
	if (!str[0] && flags.type == 'c') // Should check that handling '\0' is in acordance with the ft_flchar function
		len += 1;
	len = write(1, str, len);
	free(str);
	return (len);
...
t_flags	ft_getflags(const char **format)
{
	...
	while (**format && ft_strchr("cspdiuxX%", **format) == NULL)
	{
		...
		if(**format == '+')
			result.plus = 1;
		if(**format >= '1' || **format<= '9') //falta assegurarse de que els numeros estan ficats al final de le flags.
